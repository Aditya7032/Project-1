...
# === Fetch News (NewsAPI) ===
@st.cache_data
def fetch_news(api_key, query, page_size=20, from_date=None):
...
# === Fetch News with Subcategories, User Query, and Date Range ===
    ...
    # Date filter
    date_range = st.selectbox("Select Date Range", ["Any time", "Last 1 day", "Last 7 days", "Last 30 days"])
    from_date = None
    ...
    # Specific query for subtopics
    if category == "Entertainment":
        if user_query:
            # Adjust query to focus on movies
            final_query = f"movie '{user_query}'"
        else:
            final_query = "Entertainment"
    elif category == "Politics":
        final_query = "Indian Politics"
        if user_query:
    ...
    if st.button("Fetch News"):
        articles = fetch_news(NEWS_API_KEY, final_query, from_date=from_date)
        if articles:
